...
export class SparkService {
  async ensureAccount(userId: string, tx: Prisma.TransactionClient = prisma) {
  ...
  async grantSignupBonus(userId: string) {
    const existing = await prisma.sparkLedger.findFirst({
  ...
  async apply(input: {
    ...
    return prisma.$transaction((tx) => this.applyIn(tx, input));
  }
  ...
  async listLedger(userId: string, take = 50) {
    const rows = await prisma.sparkLedger.findMany({
      where: { userId },
