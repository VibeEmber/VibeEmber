...
import { tooSimilar } from "./normalize";
import { notifyService } from "./notify";
import type { SparkService } from "./spark";
...
export class TasksService {
  ...
  async create(
    ...
    if (!project || project.ownerId !== user.id) throw forbidden("只能给自己的产品发起助燃");
    if (project.status !== "approved") throw badRequest("产品通过审核后才能发起助燃");
    if (!this.hasExperienceEntry(project)) {
      throw badRequest("产品还没有可体验入口，先补齐链接或体验码");
    }
    const screenshotCount = project.assets.filter((asset) => asset.kind === "screenshot").length;
    if (screenshotCount < 1) throw badRequest("请先给产品上传至少 1 张截图再发起助燃");
    if (project.helpNeeded.trim().length < 20) {
      throw badRequest("请先把产品「现在最需要的帮助」写到至少 20 字");
    }
  ...
  async claim(user: SessionUser, taskId: string) {
    await creditService.assertCanClaim(user.id);
    const todayStart = new Date();
    todayStart.setHours(0, 0, 0, 0);
    const todayCount = await prisma.taskClaim.count({
      where: { userId: user.id, claimedAt: { gte: todayStart } },
    });
    if (todayCount >= 10) throw badRequest("今天领取太多了，明天再来");

    const task = await prisma.task.findUnique({ where: { id: taskId } });
    if (!task) throw notFound("助燃任务不存在");
    if (task.ownerId === user.id) throw badRequest("不能领取自己的助燃");
    if (task.status !== "open" || task.deadline < new Date()) {
      throw badRequest("助燃已不可领取");
    }
    const existed = await prisma.taskClaim.findUnique({
      where: { taskId_userId: { taskId, userId: user.id } },
    });
    if (existed && existed.status !== "cancelled") {
      throw badRequest("你已经领取过这个助燃");
    }
    ...
    const claim = await prisma.$transaction(async (tx) => {
      ...
      if (!current || current.status !== "open") throw badRequest("助燃已不可领取");
      if (current.claimedCount >= current.quota) throw badRequest("名额已满");
      const saved = existed
  ...
  async submit(user: SessionUser, claimId: string, answers: string[], screenshotKey: string) {
    ...
    if (!claim || claim.userId !== user.id) throw forbidden("只能提交自己的领取");
    if (claim.status !== "claimed") throw badRequest("当前状态不能提交");
    if (claim.submitBy < new Date()) throw badRequest("已超过提交时限");
    ...
    if (
      ...
      throw badRequest("反馈不能照抄产品介绍或任务说明");
    }
    const previous = await prisma.taskClaim.findMany({
  ...
  async cancel(user: SessionUser, claimId: string) {
    ...
    if (!claim || claim.userId !== user.id) throw forbidden("只能取消自己的领取");
    if (claim.status !== "claimed") throw badRequest("已提交的领取不能取消");
  ...
  async review(
    ...
    if (!claim) throw notFound("领取不存在");
    if (claim.task.ownerId !== user.id) throw forbidden("只有发起人可以验收");
    if (claim.status !== "submitted") throw badRequest("只能验收已提交的反馈");
    ...
    if (action === "rejected") {
      ...
      await creditService.adjust(claim.userId, CREDIT.rejectDelta);
      await notifyService.push({
        userId: claim.userId,
        type: "task_rejected",
        title: "助燃反馈未通过",
  ...
  async report(user: SessionUser, claimId: string, reason: string) {
    const claim = await prisma.taskClaim.findUnique({ where: { id: claimId } });
    if (!claim || claim.userId !== user.id) throw forbidden("只能举报自己的领取");
    if (claim.status !== "rejected") throw badRequest("只有被驳回的领取才能举报");
    const existing = await prisma.taskReport.findFirst({
      where: { claimId, reporterId: user.id, status: "pending" },
    });
    if (existing) throw badRequest("已有处理中的举报");
  ...
  async expireDue() {
    ...
    for (const claim of staleClaims) {
      await this.releaseClaim(claim.id, "cancelled");
      await creditService.adjust(claim.userId, CREDIT.timeoutDelta);
    }
  ...
  private async releaseClaim(claimId: string, status: "cancelled") {
    const claim = await prisma.taskClaim.findUnique({ where: { id: claimId } });
    if (!claim) return;
    await prisma.taskClaim.update({ where: { id: claimId }, data: { status } });
    await this.releaseSlot(claim.taskId);
  }
  ...
  async settleAccept(claimId: string, options: { auto: boolean; note: string }) {
    ...
    await creditService.adjust(claim.userId, CREDIT.acceptDelta);
    await notifyService.push({
      userId: claim.userId,
      type: "task_accepted",
  ...
  async closeByOwner(user: SessionUser, taskId: string) {
    const task = await prisma.task.findUnique({ where: { id: taskId } });
    if (!task) throw notFound("助燃任务不存在");
    if (task.ownerId !== user.id) throw forbidden("只能结束自己发起的助燃");
    if (task.status !== "open" && task.status !== "full") {
      throw badRequest("这场助燃已经结束");
    }
    const pending = await prisma.taskClaim.count({
      where: { taskId, status: "submitted" },
    });
    if (pending > 0) throw badRequest("还有待验收的反馈，先处理完再结束");
    const openClaims = await prisma.taskClaim.findMany({
...
export const tasksService = new TasksService(sparkService, storage);
